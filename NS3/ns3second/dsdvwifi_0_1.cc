...
void experiment (bool enableCtsRts,uint16_t nwifisource,bool tracing,bool saturated,std::string filename)
{
  ...
  {
    lossModel->SetLoss (nodes.Get (i)->GetObject<MobilityModel> (), nodes.Get (nwifisource + i)->GetObject<MobilityModel> (), 70); // set symmetric loss 0 <-> 1 to 50 dB
  }
  ...
  mobility.SetMobilityModel ("ns3::RandomWalk2dMobilityModel","Speed", StringValue ("ns3::UniformRandomVariable[Min=5.0|Max=20.0]"));
