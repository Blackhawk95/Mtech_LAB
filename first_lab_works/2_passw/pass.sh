...
#check input                                                                                                          
echo ${#PASSWORD}                                                                                                     
if [[ ${#PASSWORD} -ge 8 && "$PASSWORD" == *[a-z]* && "$PASSWORD" == *[0-9]* && "${PASSWORD//[^@#$%&*+=-]/}" ]]; then #Verify the char combo
...
#        done
#    done
#elif [[ `[ -f $SOURCE$WORDS ] && echo "1" || echo "0"` ]]; then #Verify if cracklib exist
#    for((i=4;i<=${#PASSWORD};i++));
#    do
#        for((j=0;j<=${#PASSWORD}-$i;j++))
#        do
...
if [[ `[ -f $ATLEAST4 ] && echo "1" || echo "0"` ]]; then #Verify if cracklib exist
     ...
     do
         for((j=0;j<=${#PASSWORD}-$i;j++))
         do
             if [ `grep -c -E -i "^${PASSWORD:$j:$i}$" $SOURCE$WORDS` -gt 0 ] #verify with dictionary (our dictionary)
            then
                echo "$PASSWORD: Weak PASSWORD"
                exit
            fi
